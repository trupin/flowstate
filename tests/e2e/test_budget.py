...
def test_budget_bar_updates(
    ...
    mock_subprocess.configure_node(
        "start", NodeBehavior.slow(duration_lines=5, summary="Initialized.")
    )
    mock_subprocess.configure_node(
        "work", NodeBehavior.slow(duration_lines=5, summary="Work done.")
    )
    mock_subprocess.configure_node(
        "done", NodeBehavior.slow(duration_lines=5, summary="Finalized.")
    )
