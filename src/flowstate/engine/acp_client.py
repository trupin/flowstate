...
class AcpHarness:
    ...
    def __init__(self, command: list[str], env: dict[str, str] | None = None) -> None:
        ...
        self._env = env
        # Track active sessions for cancel/kill
    ...
    async def kill(self, session_id: str) -> None:
        """Send session/cancel and terminate the subprocess."""
        ...
        try:
            async with spawn_agent_process(
                ...
                *cmd_args,
                ...
                # Track for kill()
                acp_session = _AcpSession(conn=conn, process=process, queue=queue)
...
class _AcpSession:
    """Tracks an active ACP session for cancel/kill support."""
    ...
    async def cancel_and_terminate(self) -> None:
        ...
        try:
            # Attempt to cancel via ACP protocol
            if hasattr(self._conn, "cancel"):
                await self._conn.cancel(session_id="")  # type: ignore[union-attr]
        except (RequestError, Exception):
            logger.debug("session/cancel failed, terminating process directly")
