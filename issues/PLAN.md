...
| UI-038 | Remove graph relayout on node click (dead code after UI-035)   | ui     | P1       | —          | done   |
| UI-039 | Add auto-follow toggle button in log viewer header             | ui     | P1       | —          | done   |
...
| ENGINE-037 | Remove SubprocessManager — fix messaging + interrupt       | engine | P0       | ENGINE-035            | done   |
...
| Issue      | Title                                                     | Domain | Priority | Depends On | Status |
| ---------- | --------------------------------------------------------- | ------ | -------- | ---------- | ------ |
| DSL-013    | Rename `tasks` attribute to `subtasks` in grammar/AST/parser | dsl | P1       | —          | done   |
| ENGINE-042 | Rename `tasks` references to `subtasks` in executor/context  | engine | P1    | DSL-013    | done   |
...
| ENGINE-052 | Fix message re-invocation after interrupt (dead ACP session) | engine | P0    | —           | done |
| ENGINE-053 | Support retry/skip on cancelled flows (re-create executor) | engine | P0     | —           | done |
| SERVER-018 | Handle retry/skip when no active executor exists          | server | P0       | ENGINE-053  | done |
| UI-049     | Add draggable resize handle between graph and log panel   | ui     | P1       | —           | done |
| UI-050     | Fix Clear button in log viewer (onClear prop not wired)   | ui     | P1       | —           | done |
| UI-051     | Fix rerun button staying visible after hover in task queue | ui     | P1       | —           | done |
...
| UI-053     | Show subtask count badge on graph node pills              | ui     | P1       | —           | done |
...
| UI-076  | Tauri auto-updater + GitHub Releases manifest      | ui          | P2       | UI-074, UI-077                              | done   |
...
| UI-081  | Directory picker title bar rendering artifact (sparkle overlay)   | ui | P2 | UI-074                                  | in_progress |
